...
if uploaded_file is not None:
    try:
        ...
        # ==========================================
        # ==========================================
        # [NEW TAB 8: GLOBAL PERFORMANCE GAP (Vertical Bullet Chart - Ultimate Clean Labels)]
        # ==========================================
        with tab_gap:
            st.subheader("8. 全局績效面專驗看板 (Professional Vertical Bullet Chart)")
            st.info("💡 📊 橫向藍色背景帶為低目標區 (<=85%)。長條圖中間數值為「實際績效」，X軸底部 🎯 標示為「理論設定績效」。\n🚨 實際比理論低 >= 10% 顯示深紅柱；⚠️ 設定績效 <= 85% 且未達標顯示紅柱。")
            ...
            if '設定績效%' in filtered_df.columns:
                ...
                if not gap_df.empty:
                    ...
                    # ĐÃ SỬA: Dời số Lý thuyết xuống trục X kèm icon 🎯 để không bao giờ bị đè chữ
                    df_gap_comp['Display_Label'] = df_gap_comp.apply(
                        lambda row: f"{row['塗料編號']}<br>({row['用途']})<br>🎯 {row['設定績效%']:.1f}%", axis=1
                    )
                    ...
                    # 1. BULLET ACT BAR: Cột đứng hiển thị hiệu suất thực tế
                    ...
                    fig_gap.add_trace(go.Scatter(
                        ...
                        mode='markers', # ĐÃ SỬA: Tắt text lơ lửng trên vạch đen
                    ...
                    # 3. BULLET BACKGROUND ZONES: Các dải màu nền
                    shapes = [
                    ...
                    fig_gap.update_layout(
                        ...
                        xaxis=dict(
                            title="<b>塗料編號 & 用途 & 目標 (ID, Usage & Target)</b>", 
                            tickangle=0, # ĐÃ SỬA: Cho chữ nằm ngang hoàn toàn để dễ đọc
                            automargin=True,
                        ...
                        margin=dict(b=160, t=100, r=40, l=80) # ĐÃ SỬA: Tăng lề dưới (b=160) để chứa 3 dòng text trục X
                    )
