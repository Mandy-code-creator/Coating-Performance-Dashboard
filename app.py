...
st.markdown("<b>核心異常監控：僅分析超耗量 ≥ 500 之項目 (Critical Deviation Focus)</b>", unsafe_allow_html=True)
...
if uploaded_file is not None:
    try:
        ...
        st.sidebar.success(f"篩選完成：已識別出 {filtered_count} 筆超耗異常 (原 {initial_count} 筆)")

        if df.empty:
            st.warning("⚠️ 目前資料中沒有超耗量大於等於 500 的項目，請確認原始數據。")
            st.stop()
        ...
        df_filtered = df[df['年月'].isin(sel_month)]
        
        sel_line = st.sidebar.multiselect("2. 選擇線別", options=sorted(df_filtered['線別'].unique()), default=df_filtered['線別'].unique())
        df_filtered = df_filtered[df_filtered['線別'].isin(sel_line)]
        
        sel_usage = st.sidebar.multiselect("3. 選擇用途", options=sorted(df_filtered['用途'].unique()), default=df_filtered['用途'].unique())
        filtered_df = df_filtered[df_filtered['用途'].isin(sel_usage)]

        # ==========================================
        # [ 3. VISUALIZATION ]
        # ==========================================
        st.markdown(f"### 📈 異常項目深度分析 (當前分析項目數: {len(filtered_df)})")
        ...
        with tab_overview:
            ...
            k2.metric("總超耗量 (Sum of Deviation)", f"{total_delta:,.0f}", delta_color="inverse")
            ...
            with col_pie:
                ...
                fig_pie.update_layout(title="<b>超耗項目之績效分布</b>")
        ...
        with tab_pareto:
            ...
            if not pareto_df.empty:
                ...
                fig_pareto.update_layout(yaxis2=dict(title="累計%", overlaying='y', side='right', range=[0, 105]),
                                         height=600, title="<b>關鍵 80/20 改善順序 (Deviation ≥ 500)</b>")
                fig_pareto.update_xaxes(tickangle=-90)
                st.plotly_chart(fig_pareto, use_container_width=True)

        with tab_rootcause:
            st.subheader("3. 穩定度分析 (品質與操作因素)")
        ...
        with tab_scatter:
            st.subheader(f"4. 燈號全景總覽")
            plot_df = filtered_df[filtered_df['合計理論耗用'] > 0].copy()
            seq_map = {code: i+1 for i, code in enumerate(sort_order)}
            plot_df['序號'] = plot_df['塗料編號'].map(seq_map)
            fig = px.scatter(plot_df, x='序號', y='合計績效%', color='績效等級',
                             color_discrete_map=perf_color_map, size='合計理論耗用', hover_name='塗料編號')
            fig.update_layout(**common_layout, height=700, title="<b>異常項目分佈 (點越大=理論量越高)</b>")
        ...
        with tab_bar:
            ...
            for i in range(num_charts):
                ...
                fig_bar.update_layout(barmode='group', height=500, title=f"第 {i+1} 組明細")
                fig_bar.update_xaxes(tickangle=-90)
                st.plotly_chart(fig_bar, use_container_width=True)

        with tab_dev:
            st.subheader("6. 耗用差異絕對值")
            for i in range(num_charts):
                ...
                fig_dev.update_layout(height=500, title=f"第 {i+1} 組差異")
    ...
    st.info("👈 請上傳資料檔。系統將鎖定超耗量 ≥ 500 的項目進行專業化分析。")
