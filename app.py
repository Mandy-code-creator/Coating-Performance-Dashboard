    ...
    try:
        ...
        if not filtered_df.empty:
            ...
            if not worst_perf_df.empty:
                ...
                k3.metric("首要改善對象", f"{worst_row['塗料編號']}", f"{worst_row['合計績效%']:.2f}%")
        ...
        viz_tab1, viz_tab2, viz_tab3, viz_tab4, viz_tab5 = st.tabs([
            "🎯 績效燈號", "📊 耗用對比", "📉 差異分析", "🚨 異常柏拉圖 (Pareto)", "📦 穩定度分析 (Box Plot)"
        ])

        with viz_tab1:
            st.subheader(f"1. 塗料績效散佈圖 (共 {total_paints} 支)")
            st.info("**💡 讀圖提示：** 🔴 嚴重超耗 (<85%) | 🟡 注意 (85-95%) | 🔵 接近理論 (95-100%) | 🟢 達標/節省 (≥100%)。圓圈大小代表「理論耗用量」。請優先尋找**「大紅圈」**！")
            ...
            if not filtered_df.empty and total_paints > 0:
                for i in range(num_charts):
                    ...
                    plot_df = filtered_df[filtered_df['塗料編號'].isin(current_batch)].copy().dropna(subset=['合計理論耗用', '合計績效%'])
                    ...
                    if not plot_df.empty:
                        ...
                        fig = px.scatter(
                            ...
                            size='合計理論耗用', size_max=35, category_orders={"績效等級": labels}, hover_data=['線別', '用途', '合計理論耗用', '合計實際耗用']
                        )
                        
                        fig.add_hline(y=100, line_dash="dash", line_color="red", line_width=2.5)
                        fig.add_annotation(x=1.01, y=100, xref="paper", yref="y", text="<b>🎯 目標 100%</b>", showarrow=False, xanchor="left", yanchor="middle", font=dict(color="red", size=14))
        ...
        # --- 🌟 TÍNH NĂNG MỚI: TAB 4 PARETO CHART ---
        with viz_tab4:
            st.subheader("4. 異常超耗柏拉圖 (Pareto Chart) - 優先改善清單")
            st.markdown("找出造成最多浪費的關鍵少數塗料 (80/20法則)。**請優先處理累積曲線(藍線)前段的塗料**。")
            
            # Chỉ lấy các mã sơn bị hao hụt (Deviation > 0)
            ...
            if not pareto_df.empty:
                ...
                # Giới hạn hiển thị Top 40 để biểu đồ không bị quá chật
        ...
        # --- 🌟 TÍNH NĂNG MỚI: TAB 5 BOX PLOT ---
        with viz_tab5:
            col1, col2 = st.columns(2)
            with col1:
                st.subheader("5. 供應商穩定度分析 (Box Plot)")
                st.markdown("觀察各廠商的績效分布範圍，**盒子越長代表品質越不穩定**。")
