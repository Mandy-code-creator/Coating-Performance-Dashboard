...
st.markdown("依據 MES/Excel 數據進行系統化分析 (修復 NaN 與多線別版)")
...
if uploaded_file is not None:
    try:
        # 讀取資料 (dtype=str để tránh lỗi đoán sai kiểu dữ liệu ban đầu)
        ...
        df = df.dropna(subset=['塗料編號', '線別'])
        
        # 💡 SỬA LỖI QUAN TRỌNG: Loại bỏ các dòng tiêu đề bị lặp lại trong file (nếu có)
        df = df[df['線別'] != '線別']

        # 2. 格式化文字欄位
        cat_cols = ['線別', '油漆廠商', '顏色', '樹脂', '用途', '年月', '塗料編號']
        for col in cat_cols:
            if col in df.columns:
                df[col] = df[col].fillna('未定義').astype(str).str.strip()

        # 3. 數值轉換與清理 (Xử lý dấu phẩy và ép kiểu số)
        ...
        if not filtered_df.empty:
            ...
            worst_row = filtered_df.loc[filtered_df['合計績效%'].idxmin()] if filtered_df['合計績效%'].notnull().any() else None
        ...
        with tab1:
            ...
            # Loại bỏ NaN trong '合計理論耗用' và '合計績效%' trước khi vẽ Scatter
            plot_df = filtered_df.dropna(subset=['合計理論耗用', '合計績效%']).copy()
            # Kích thước phải dương (>0)
            ...
            if not plot_df.empty:
                ...
                fig_scatter.update_layout(xaxis=dict(dtick=1, tickangle=-45, categoryarray=sort_order), height=700)
                st.plotly_chart(fig_scatter, use_container_width=True)
            else:
                st.warning("⚠️ 沒有足夠的數據繪製散佈圖 (數據可能為空或理論值為 0)。")
        ...
        with tab2:
            ...
            fig_bar.update_layout(barmode='group', xaxis=dict(dtick=1, tickangle=-45, categoryarray=sort_order), height=650)
        ...
        with tab3:
            ...
            fig_dev.update_layout(xaxis=dict(dtick=1, tickangle=-45, categoryarray=sort_order), height=650)
        ...
        with tab4:
            ...
            with c1:
                ...
                st.plotly_chart(px.line(df_trend, x='年月', y='合計績效%', markers=True, title="月度趨勢"), use_container_width=True)
            with c2:
                df_sup = filtered_df.groupby('油漆廠商')['合計績效%'].mean().reset_index()
                st.plotly_chart(px.bar(df_sup, x='油漆廠商', y='合計績效%', title="供應商績效"), use_container_width=True)
